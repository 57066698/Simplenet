...
class Batch_Normalization2D(Layer):
    def __init__(self, in_channels, momentum=0.99, epsilon=1e-12):
        ...
        self.weights["gamma"] = np.ones((1, in_channels, 1, 1))
        self.weights["beta"] = np.zeros((1, in_channels, 1, 1))
        self.weights["running_mean"] = np.zeros((1, in_channels, 1, 1))
        self.weights["running_var"] = np.ones((1, in_channels, 1, 1))
    ...
    def __call__(self, x):
        ...
        assert len(x.shape) == 4 and x.shape[1] == in_channels
        num = np.prod(x.shape, dtype=np.float) / in_channels

        if self.mode == "train":
            # 1
            mean = np.sum(x, axis=(0, 2, 3), keepdims=True) / num  # [1, D, 1, 1]
            # 2
            xmean = x - mean  # [N, D, H, W]
            # 3
            xmean_square = xmean ** 2  # [N, D, H, W]
            # 4
            var = np.sum(xmean_square, axis=(0, 2, 3), keepdims=True) / num  # [1, D, 1, 1]
            # 5
            sqrtvar = np.sqrt(var + epsilon)  # [1, D, 1, 1]
            # 6
            invvar = 1. / sqrtvar  # [1, D, 1, 1]
            # 7
            x_hat = xmean * invvar  # [N, D, H, W]
            # 8
            y = gamma * x_hat + beta  # [N, D, H, W]
    ...
    def backwards(self, da):
        ...
        num = np.prod(da.shape, dtype=np.float) / in_channels

        # 8
        dx_hat = da * gamma  # [N, D, H, W]
        dgamma = np.sum(da * x_hat, axis=(0, 2, 3), keepdims=True)  # [1, D, 1, 1]
        dbeta = np.sum(da, axis=(0, 2, 3), keepdims=True)  # [1, D, 1, 1]

        # 7
        dxmean1 = dx_hat * invvar  # [N, D, H, W] * [1, D, 1, 1] = [N, D, H, W]
        dinvvar = np.sum(dx_hat * xmean, axis=(0, 2, 3), keepdims=True)  # [1, D, 1, 1]
        ...
        # 4
        dxmean_square = np.ones((N, dim, H, W)) * dvar / num  # [N, D]
        ...
        # 1
        dx2 = np.ones((N, dim, H, W)) * dmean / N  # [N, D]
        dx = dx1 + dx2
